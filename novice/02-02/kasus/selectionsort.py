...
def selection_sort(integers):
    """Performs in-place Selection Sort on a list of integers."""
    ...
    for i in range(length - 1):
        ...
        # set initial position as min val, index
        minimal_value = integers[i]
        minimal_index = i

        for j in range(i + 1, length):
            if integers[j] < minimal_value:
                minimal_value = integers[j]
                minimal_index = j

        integers[i], integers[minimal_index] \
            = integers[minimal_index], integers[i]
